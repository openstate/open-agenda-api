...
class CentraalMuseumExtractor(BaseExtractor, HttpRequestMixin):
    def get_collection_objects(self):
        url = self.source_definition['url']

        all_links_xpath = ".//ul[contains(@class,'itemList')]/li"

        resp = self.http_session.get('%s/bezoeken/agenda/' % (url))
